<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="edm:Schema[@Namespace='microsoft.graph']">
        <xsl:copy>
            ...
            <xsl:choose>
                <xsl:when test="not(edm:Annotations[@Target='microsoft.graph.GraphService/identityGovernance/entitlementManagement/accessPackageAssignments'])">
                    <xsl:element name="Annotations">
                        <xsl:attribute name="Target">microsoft.graph.GraphService/identityGovernance/entitlementManagement/accessPackageAssignments</xsl:attribute>
                        <xsl:call-template name="DeleteRestrictionsTemplate">
                            <xsl:with-param name="deletable">false</xsl:with-param>
                        </xsl:call-template>
                    </xsl:element>
