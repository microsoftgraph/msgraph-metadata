...
.Example
    ./scripts/copy-annotations-to-csdl.ps1 -sourceCsdlPath C:/github/msgraph-metadata/schemas/annotated-v1.0-Prod.csdl -targetCsdlPath C:/github/msgraph-metadata/schemas/annotated-v1.0-Prod.csdl
...
param(
    [Parameter(Mandatory=$true)][string]$sourceCsdlPath,
    [Parameter(Mandatory=$true)][string]$targetCsdlPath
)

if (-not (Test-Path $sourceCsdlPath)) {
    throw "Source CSDL file does not exist: $sourceCsdlPath"
}
if (-not (Test-Path $targetCsdlPath)) {
    throw "Target CSDL file does not exist: $targetCsdlPath"
}
# Load the source CSDL file
$sourceCsdl = [xml](Get-Content $sourceCsdlPath)
...
Write-Host "Annotations copied from $sourceCsdlPath to $targetCsdlPath" -ForegroundColor Green